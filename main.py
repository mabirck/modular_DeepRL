...
def main():
    ...
    for k, ev in enumerate(args.env_name):
        current_games.append(ev)

        # SELECTOR FOR CONTINUAL REGIME
        if ev == "RoboschoolHumanoidFlagrun-v1":
            continual_path = args.algo+"_RoboschoolHumanoid-v1_"+args.act_func+'_'+str(args.seed)+'.pt'
        elif ev == "RoboschoolHumanoidFlagrunHarder-v1":
            continual_path = args.algo+"_RoboschoolHumanoidFlagrun-v1_"+args.act_func+'_'+str(args.seed)+'.pt'
        ...
        elif continual_path != None:
            print("LOADING ", continual_path)
            actor_critic, ob_rms = \
                        torch.load(os.path.join(args.save_dir, args.algo)+'/'+continual_path)
        ...
        for j in range(num_updates):
            ...
            if args.vis and j % args.vis_interval == 0:
                try:
                    ...
                    win = visdom_plot(viz, win, args.log_dir, ev, args.algo)
                except IOError:
                    pass
